...
public class UploadSave extends HttpServletRequestWrapper {
  ...
  public static String saveFile(FileItem fileItem, String upDir) {
    ...
    // 순수 파일명 추출  
    //getName() 은 파일경로까지 전부 준다.
    filename = fileItem.getName().substring(idx + 1);  //idx + 1 : 마지막 idx 경로 다음에 (/____.jpg)
    ...
    try {
      ...
      // storage 폴더에 파일명 저장 :
    //uploadedFile : 경로와, 순수파일 명을 가지고 있다.
